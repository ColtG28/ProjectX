...
mod tests {
    ...
    use parser_fixtures::{build_test_fat_macho, build_test_macho, MachoSegmentSpec};

    #[test]
    fn parses_fat_macho_wrapping_a_thin_binary() {
        ...
        let fat = build_test_fat_macho(&thin);
        let parsed = parse_binary(&fat).unwrap();
        assert!(parsed
            .segments
            .iter()
            .any(|segment| segment.name == "__text" || segment.name == "__data"));
        assert!(parsed
            .dylibs
            .iter()
            .any(|dylib| dylib.path.contains("libsystem")));
    }
}
