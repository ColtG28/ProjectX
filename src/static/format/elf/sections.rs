...
mod tests {
    ...
    use super::{check, parse_sections};
    use parser_fixtures::{build_test_elf, malformed_elf_bad_bounds, malformed_elf_bad_shstrtab};

    #[test]
    fn parses_real_elf_section_headers() {
        let bytes = build_test_elf(&[".text", ".interp", ".shstrtab"], None, b"");
        let sections = parse_sections(&bytes).unwrap();
        assert!(sections.iter().any(|section| section.name == ".interp"));
        assert!(sections.iter().any(|section| section.name == ".shstrtab"));
        assert!(sections.iter().any(|section| section.index == 1));
    }

    #[test]
    fn dynamic_loader_chain_message_is_clear() {
        let bytes = build_test_elf(
            &[".dynamic", ".dynsym", ".interp", ".shstrtab"],
            Some("/lib64/ld-linux-x86-64.so.2"),
            b"dlopen dlsym mprotect /proc/self/exe execve",
        );
    ...
    #[test]
    fn malformed_elf_headers_fail_safely() {
        assert!(check(b"\x7fELF").is_empty());
    }

    #[test]
    fn invalid_section_bounds_fail_safely() {
        let bytes = malformed_elf_bad_bounds();
        assert!(check(&bytes).is_empty());
        assert!(parse_sections(&bytes).is_none());
    }

    #[test]
    fn broken_shstrtab_fails_safely() {
        let bytes = malformed_elf_bad_shstrtab();
        assert!(check(&bytes).is_empty());
        assert!(parse_sections(&bytes).is_none());
    }
}
