...
mod tests {
    ...
    #[test]
    fn identifies_chatgpt_compressed_localizations_as_bundle_resources() {
        let context = apple_bundle_resource_context(
            Path::new("/Applications/ChatGPT.app/Contents/Frameworks/Assets.framework/Versions/A/Resources/Assets_Assets.bundle/Contents/Resources/CompressedStrings/am.json.lzfse"),
            "am.json.lzfse",
            "lzfse",
        );
        assert!(context.is_known_resource);
        assert!(context.suppress_magic_mismatch);
        assert!(context.suppress_high_entropy);
    }

    #[test]
    fn identifies_small_bundle_metadata_files() {
        let context = apple_bundle_resource_context(
            Path::new("/Applications/Aiming.Pro.app/.localized"),
            ".localized",
            "",
        );
        assert!(context.is_known_resource);
