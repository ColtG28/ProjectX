...
mod tests {
    ...
    #[test]
    fn app_bundle_detection_requires_directory_with_app_extension() {
        let root =
            std::env::temp_dir().join(format!("projectx_bundle_test_{}", std::process::id()));
        let bundle = root.join("Example.app");
        fs::create_dir_all(&bundle).expect("bundle dir");
        assert!(is_app_bundle(&bundle));
        fs::remove_dir_all(root).ok();
    }

    #[test]
    fn bundle_plan_finds_primary_executable_and_helpers() {
        ...
        fs::create_dir_all(&helpers).expect("helpers dir");
        fs::write(
            bundle.join("Contents").join("Info.plist"),
            r#"<plist><dict><key>CFBundleExecutable</key><string>Example</string></dict></plist>"#,
        )
        .expect("plist");
        fs::write(macos.join("Example"), b"primary").expect("primary");
        fs::write(helpers.join("HelperTool"), b"helper").expect("helper");

        let plan = resolve_app_bundle_plan(&bundle).expect("bundle plan");
        assert_eq!(plan.primary_executable, macos.join("Example"));
        assert_eq!(plan.helper_executables, vec![helpers.join("HelperTool")]);
    ...
    #[cfg(unix)]
    #[test]
    fn bundle_plan_skips_symlinked_components() {
        ...
        fs::create_dir_all(&macos).expect("macos dir");
        fs::write(macos.join("Example"), b"primary").expect("primary");
        symlink(macos.join("Example"), macos.join("LinkedHelper")).expect("symlink");

        let plan = resolve_app_bundle_plan(&bundle).expect("bundle plan");
        assert!(plan
            .skipped_items
            .iter()
            .any(|note| note.contains("symlinked")));
