...
const EMBEDDED_WEIGHTS: [f32; 74] = [
    ...
    0.06, 0.07, 0.07, 0.07, 0.07, 0.08, 0.08, 0.08, 0.08, 0.09, 0.09, 0.10, 0.10,
];

#[cfg(test)]
mod tests {
    use super::{sigmoid, PortableModel};

    #[test]
    fn predicts_binary_scores() {
        let model = PortableModel {
            model_type: "portable-linear-v1".to_string(),
            version: "1".to_string(),
            feature_names: vec!["a".to_string(), "b".to_string()].into(),
            weights: vec![2.0, -1.0].into(),
            intercept: 0.0,
        ...
        assert!(!model.schema_matches_runtime());

        let clean_score = model
            .weights
            .iter()
            .zip([0.0f32, 2.0].iter())
            .fold(model.intercept, |acc, (weight, value)| {
                acc + (weight * value)
            });
        let bad_score = model
            .weights
        ...
        assert!(sigmoid(clean_score) < model.suspicious_threshold);
        assert!(sigmoid(bad_score) >= model.malicious_threshold);
    }
}
